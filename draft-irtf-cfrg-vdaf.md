...
sequence of candidate prefixes has at most one non-zero entry, and that the
non-zero entry is `1`. Indeed, for an honestly generated Poplar1 report, we
expect the IDPF tree has exactly one non-zero path from the root to the leaves,
but this is not guaranteed by the secure sketch alone.

IDPFs are designed to prevent a malicious Client from producing key shares with
more than one non-zero path in the tree. This property is formalized as
"extractability" in Appendix D of {{BBCGGI21}}: for any pair of key shares and
public share produced by a malicious Client and a given level of the tree, one
can efficiently extract from the Client the underlying prefix (i.e., a prefix
of `alpha`), in particular from its queries to the random oracle.
...
produce IDPF shares with two non-zero nodes.
